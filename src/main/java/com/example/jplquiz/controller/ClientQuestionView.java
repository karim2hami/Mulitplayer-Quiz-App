...
public class ClientQuestionView implements Initializable {
  ...
  /**
   * @author devinhasler gives the GUI Buttons the right colors so that the user recieves a recall
   *     for his answer
   */
  ...
  public void loadQuestionFromList() {
    ...
    if (questionsNumber < questionModels.size()) {
      ...
      imgQuestion.setImage(
          new Image(String.valueOf(getClass().getResource(questionModel.imagePath()))));
      countDownTimer();
      questionsNumber++;
  ...
  /**
   * @author devinhasler Creates a Timer that shows the right and wrong answers via GUI for 2
   *     seconds
   */
  @FXML
  ...
  /**
   * @author karimtouhami Changes UI from ClientQuestionView to ServerRanking and initialises
   *     listenForRankings
   */
  @FXML
  void changeToServerRanking() {
    try {
      ...
      FXMLLoader fxmlLoader =
          new FXMLLoader(getClass().getResource("/com/example/jplquiz/server-ranking.fxml"));
